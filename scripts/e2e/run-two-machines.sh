...
# ui_prefix <key>：herdr 的 prefix（ctrl+b）+ 一个键。tmux 的 `C-b` 键名送不到 herdr，
# 直接写原始字节 0x02 才行（真 TUI 实测）。
ui_prefix() {
  docker exec "${A}" tmux -L hf send-keys -t ui -H 02
  sleep 0.3
  ui_keys "$1"
}
...
a_hook_reloaded() {
  local n=""
  n="$(ax "grep -c '已自动 reload-config' '${A_STATE}/logs/forward.log' 2>/dev/null || true")"
  [[ ${n} =~ ^[1-9] ]] && printf 'yes\n'
  return 0
}
...
${COPY_PLUGIN}
herdr plugin link ${A_HOME}/plugin >/dev/null"
...
t_describe "前置：B 在「server 运行中」装插件（startup hook 未跑，键位未装）；A 的插件已就绪"
...
wait_for 20 a_hook_reloaded
t_exit_ok 0 "$?" "A 的 startup hook 装好键位后自动 reload-config（不用手动重载）"
...
# --- 4. 打开真 herdr TUI ----------------------------------------------------------
t_describe "A：打开 herdr（tmux 里的真 TUI client）"
...
t_exit_ok 0 "$?" "x 关闭面板（暂停时按的键直接生效）"
wait_for 20 a_bridge_state_is connected
t_exit_ok 0 "$?" "A 的桥接 connected"
...
t_exit_ok 0 "$?" "输入现在落在 B 的 shell"
ui_prefix f
wait_for 10 ui_shows 'CLIENT  laptop 已连接'
t_exit_ok 0 "$?" "B 的面板：client laptop 已连接"
screen="$(ui_screen)"
...
t_exit_ok 0 "$?" "A GET localhost:5173 = B 的页面"
...
ui_keys x
wait_for 5 ui_hides 'Port Forward'
t_exit_ok 0 "$?" "关闭 B 的面板"
wait_for 20 tab_bar_has "⇅5173"
t_exit_ok 0 "$?" "tab bar 显示 ⇅5173（A 的 tab bar 条目由 herdr 在 B 上执行）"
...
t_exit_ok 0 "$?" "回到 Local"
ui_prefix f
wait_for 10 ui_shows '\[✓\] 1\. devbox'
t_exit_ok 0 "$?" "面板显示 devbox 为当前活动"
screen="$(ui_screen)"
