...
if [ -x "${bin}" ]; then exec "${bin}" internal diagnose-panel "$@"; fi
if command -v go >/dev/null 2>&1 && [ -f "${src}/go/go.mod" ]; then
  (cd "${src}/go" && HERDR_PLUGIN_ROOT="${HERDR_PLUGIN_ROOT:-${here}}" GOFLAGS=-mod=vendor exec go run ./cmd/forward internal diagnose-panel "$@")
  exit $?
fi
