...
if [ -x "${bin}" ]; then HERDR_PLUGIN_ROOT="${HERDR_PLUGIN_ROOT:-${here}}" exec "${bin}" bootstrap "$@"; fi
if command -v go >/dev/null 2>&1 && [ -f "${src}/go/go.mod" ]; then
  cd "${src}/go" || exit 1
  HERDR_PLUGIN_ROOT="${HERDR_PLUGIN_ROOT:-${here}}" GOFLAGS=-mod=vendor exec go run ./cmd/forward bootstrap "$@"
  exit $?
fi
