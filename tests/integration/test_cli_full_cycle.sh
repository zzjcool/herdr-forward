...
t_eq "up" "$(t2_state '.forwards[0].status')" "status up"
...
t_it "第一次请求回环成功"
t_eq "pong" "$(t2_roundtrip "${LOCAL_PORT}" "cli-cycle-1")" "tunneled echo reply"
t_it "第二次请求仍回环（转发稳定）"
t_eq "pong" "$(t2_roundtrip "${LOCAL_PORT}" "cli-cycle-2")" "second tunneled reply"
...
t_eq "1" "$(t2_state '.forwards | length')" "活记录保留"
...
t_eq "down" "$(t2_state '.forwards[0].status')" "状态被强制为 down"
t2_run "${FORWARD_BIN}" doctor --fix
t_eq "0" "${T2_RC}" "doctor --fix exit 0"
t_eq "up" "$(t2_state '.forwards[0].status')" "状态修复为 up"
...
TUNNEL_ID=""
t_it "状态记录已删空"
t_eq "0" "$(t2_state '.forwards | length')" "无记录残留"
t_it "本地端口不再监听"
sleep 0.4
...
t2_run "${FORWARD_BIN}" doctor --prune
t_eq "0" "${T2_RC}" "doctor --prune exit 0"
t_eq "0" "$(t2_state '.forwards | length')" "死记录被清"
TUNNEL_ID=""
